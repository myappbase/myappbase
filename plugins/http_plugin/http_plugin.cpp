#include <http_plugin/http_plugin.hpp>
#include <help_http/help_http.hpp>

#ifdef BOOST_ASIO_HAS_LOCAL_SOCKETS

#include <help_http/local_endpoint.hpp>

#endif

#include <plugin_help/exceptions.hpp>
#include <plugin_help/thread_utils.hpp>
...
namespace my {
    ...
    public:
        ...
        optional<my::plugin_help::named_thread_pool> thread_pool;
        ...
        bool host_is_valid(const std::string &host, const string &endpoint_local_host_port, bool secure) {
            ...
            static auto has_port_expr = regex(
                    "[^:]:[0-9]+$"); /// ends in :<number> without a preceeding colon which implies ipv6
            if (std::regex_search(host, has_port_expr)) {
                ...
                return host_port_is_valid(host + ":" + std::to_string(
                        secure ? websocketpp::uri_default_secure_port : websocketpp::uri_default_port),
                                          endpoint_local_host_port);
            }
        }

        ssl_context_ptr on_tls_init(websocketpp::connection_hdl hdl) {
            ssl_context_ptr ctx = websocketpp::lib::make_shared<websocketpp::lib::asio::ssl::context>(
            ...
            try {
                ...
                fc::ec_key ecdh = EC_KEY_new_by_curve_name(
                        https_ecdh_curve == SECP384R1 ? NID_secp384r1 : NID_X9_62_prime256v1);
                if (!ecdh)
                    MY_THROW(my::plugin_help::http_exception, "Failed to set NID_secp384r1");
                if (SSL_CTX_set_tmp_ecdh(ctx->native_handle(), (EC_KEY *) ecdh) != 1)
                    MY_THROW(my::plugin_help::http_exception, "Failed to set ECDH PFS");
                ...
                if (SSL_CTX_set_cipher_list(ctx->native_handle(), \
                  ...
                  "!DHE:!RSA:!AES128:!RC4:!DES:!3DES:!DSS:!SRP:!PSK:!EXP:!MD5:!LOW:!aNULL:!eNULL") != 1)
                    MY_THROW(my::plugin_help::http_exception, "Failed to set HTTPS cipher list");
                ...
                try {
                    ...
                    error_results results{websocketpp::http::status_code::internal_server_error,
                                                         ...
                                                         error_results::error_info(
                                                                 fc::exception(FC_LOG_MESSAGE(error, e.what())),
                                                                 verbose_http_errors)};
                    con->set_body(fc::json::to_string(results, deadline));
                } catch (...) {
                    ...
                    error_results results{websocketpp::http::status_code::internal_server_error,
                                                         ...
                                                         error_results::error_info(
                                                                 fc::exception(
                                                                         FC_LOG_MESSAGE(error, "Unknown Exception")),
                                                                 verbose_http_errors)};
                    con->set_body(fc::json::to_string(results, deadline));
                }
        ...
        bool verify_max_bytes_in_flight(const T &con) {
            if (bytes_in_flight > max_bytes_in_flight) {
                ...
                error_results results{websocketpp::http::status_code::too_many_requests, "Busy",
                                      error_results::error_info()};
                ...
                if (handler_itr != url_handlers.end()) {
                    ...
                    app().post(appbase::priority::low,
                               [&ioc = thread_pool->get_executor(), &bytes_in_flight = this->bytes_in_flight,
                                       handler_itr, this, resource{std::move(resource)}, body{
                                       std::move(body)}, con]() mutable {
                                   ...
                                   try {
                                       handler_itr->second(std::move(resource), std::move(body),
                                                           [&ioc, &bytes_in_flight, con, this](int code,
                                                                                               fc::variant response_body) {
                                                               ...
                                                               if (!verify_max_bytes_in_flight(con)) {
                                                                   ...
                                                                   boost::asio::post(ioc,
                                                                                     [response_body{std::move(
                                                                                             response_body)}, response_size, &bytes_in_flight,
                    ...
                    error_results results{websocketpp::http::status_code::not_found,
                                          "Not Found", error_results::error_info(
                                    fc::exception(FC_LOG_MESSAGE(error, "Unknown Endpoint")), verbose_http_errors)};
    ...
    template<>
    bool http_plugin_impl::allow_host<detail::asio_local_with_stub_log>(
            const detail::asio_local_with_stub_log::request_type &req,
            websocketpp::server<detail::asio_local_with_stub_log>::connection_ptr con) {
        return true;
    }
    ...
    void http_plugin::set_program_options(options_description &, options_description &cfg) {
        ...
        if (current_http_plugin_defaults.default_unix_socket_path.length())
            cfg.add_options()
                    ("unix-socket-path",
                     bpo::value<string>()->default_value(current_http_plugin_defaults.default_unix_socket_path),
        ...
        cfg.add_options()
                ...
                ("http-max-response-time-ms", bpo::value<uint32_t>()->default_value(90),
    ...
    void http_plugin::plugin_initialize(const variables_map &options) {
        ...
        try {
            ...
            MY_ASSERT(my->thread_pool_size > 0, my::plugin_help::plugin_config_exception,
    ...
    void http_plugin::plugin_startup() {
        ...
        if (my->unix_endpoint) {
            try {
                ...
                fc_elog(logger, "unix socket service (${path}) failed to start: ${e}",
                        ("e", e.to_detail_string())("path", my->unix_endpoint->path()));
                throw;
            } catch (const std::exception &e) {
                fc_elog(logger, "unix socket service (${path}) failed to start: ${e}",
    ...
    void http_plugin::handle_exception(const char *api_name, const char *call_name, const string &body,
        ...
        try {
            try {
                ...
                if (e.code() != my::plugin_help::greylist_net_usage_exceeded::code_value &&
                    e.code() != my::plugin_help::greylist_cpu_usage_exceeded::code_value) {
