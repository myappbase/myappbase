...
#define MY_ASSERT( expr, exc_type, FORMAT, ... )                \
   FC_MULTILINE_MACRO_BEGIN                                           \
   if( !(expr) )                                                      \
      FC_THROW_EXCEPTION( exc_type, FORMAT, __VA_ARGS__ );            \
   FC_MULTILINE_MACRO_END

#define MY_THROW( exc_type, FORMAT, ... ) \
    throw exc_type( FC_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) );

/**
 * Macro inspired from FC_RETHROW_EXCEPTIONS
 * The main difference here is that if the exception caught isn't of type "my::chain::chain_exception"
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define MY_RETHROW_EXCEPTIONS(exception_type, FORMAT, ... ) \
   catch( const std::bad_alloc& ) {\
      throw;\
   } catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
   } catch (my::chain::chain_exception& e) { \
 ...
 * This macro will rethrow the exception as the specified "exception_type"
 */
#define MY_CAPTURE_AND_RETHROW( exception_type, ... ) \
   catch( const std::bad_alloc& ) {\
      throw;\
   } catch( const boost::interprocess::bad_alloc& ) {\
      throw;\
...
#define MY_RECODE_EXC( cause_type, effect_type ) \
...
namespace plugin_help {
    ...
    */
    ...
    FC_DECLARE_DERIVED_EXCEPTION( chain_type_exception, chain_exception,
      ...
      FC_DECLARE_DERIVED_EXCEPTION( currencyinfo_type_exception,       chain_type_exception,
      								3010017, "Invalid currencyinfo" )		//liruigang
   ...
   FC_DECLARE_DERIVED_EXCEPTION( action_validate_exception, chain_exception,
      ...
      FC_DECLARE_DERIVED_EXCEPTION( dont_modif_auth, action_validate_exception,
                                    3050012, "dont modif auth" )			//liruigang
   ...
   FC_DECLARE_DERIVED_EXCEPTION( plugin_exception, chain_exception,
      ...
      FC_DECLARE_DERIVED_EXCEPTION( missing_plugin_exception,					  plugin_exception,
                                   3110007, "plugin is not enabled")		//liruigang
   ...
   FC_DECLARE_DERIVED_EXCEPTION( contract_exception,           chain_exception,
      ...
      // lv dev
      FC_DECLARE_DERIVED_EXCEPTION( account_already_lock,          contract_exception,
                                        3160011, "account already lock" )
      FC_DECLARE_DERIVED_EXCEPTION( api_already_discard,          contract_exception,
      3160012, "api already discard" )
   ...
   FC_DECLARE_DERIVED_EXCEPTION( mongo_exception, chain_exception,
                                 3220000, "Mongo exception" )
      FC_DECLARE_DERIVED_EXCEPTION( mongo_db_insert_fail, mongo_exception,
                                 3220001, "Fail to insert new data to Mongo DB" )
      FC_DECLARE_DERIVED_EXCEPTION( mongo_db_update_fail, mongo_exception,
