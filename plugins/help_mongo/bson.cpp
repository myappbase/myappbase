                                ...
                                if (obj.size() == 1) {
                                    ...
                                    if (number->key().compare("$numberLong") == 0) {
                                        bsoncxx::types::b_date date(
                                                std::chrono::milliseconds(number->value().as_int64()));
                                        c.append(date);
                                        break;
                                    }
                                }
        ...
        void from_bson(const T &ele, fc::variant &v) {
            switch (ele.type()) {
                ...
                case bsoncxx::type::k_binary: {
                    ...
                    std::copy(ele.get_binary().bytes, ele.get_binary().bytes + ele.get_binary().size,
                              blob.data.begin());
                    v = blob;
                    return;
                }
                ...
                case bsoncxx::type::k_timestamp:
                    v = fc::variant_object("$timestamp",
                                           fc::mutable_variant_object("t", ele.get_timestamp().timestamp)("i",
                                                                                                          ele.get_timestamp().increment));
                    return;
