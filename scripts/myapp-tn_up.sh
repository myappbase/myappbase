#!/bin/bash
#
# myappbase-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# myappbase-tn_bounce.sh and myappbase-tn_roll.sh scripts.
...
datadir=var/lib/node_$MYAPP_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/myappbase/node_$MYAPP_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/myappbase/node_$MYAPP_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid
